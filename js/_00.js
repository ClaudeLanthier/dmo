...
data:{
	insert:function(r){
		var q,u,ii,v,c,tbl='',ot=[],ttl=[],tts=[],mrg=[],vl,oh=[],oj=[],s,sp,spa,spb,ra=[],spr,vlu;
		_rv={};
		_cv=false;
		if(r.RECODE){
			v=r.RECODE;
			...
			for(var i=0,l=v.length;i<l;i++){
				...
				if(oj[0]=='s' || oj[0]=='sp'){
					for(var j=0;j<oj[1];j++){
						tbl+=c+'_'+ttl[c][j]+' '+(v[i].typ||'varchar(255)')+',\n';
					}
				}
			}
			tbl+='PRIMARY KEY (id)\n';
		...
		for(var t in r){
			...
			for(var c in u){
				...
				if(oh[0]!="a" && oh[0]!='d'){	
					if(c!='undefined'){
						if(oh[0]=='s' || oh[0]=='sp' ){
							for(var j=0;j<oh[1];j++){q+=(q?',':'')+c+"_"+ttl[c][j];}
						}else{
							q+=(q?',':'')+c;
						}
					}
			...
			_.loop(r[t],function(val,i,col){
				...
				// 
					...
					for(var k=0;k<o[1];k++){
						...
						s=$.f.tools.recode(col+"_"+ttl[col][k],s,ctyp[col]);
					...
					for(var k=0;k<o[1];k++){
						...
						for(var m=0;m<sp.length;m++){					
							...
							spa=$.f.tools.recode(col+"_"+ttl[col][k],spa,ctyp[col]);
